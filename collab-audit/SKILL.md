...
- 2세션 이상
- 100+ 메시지
- **단일 세션 고밀도 예외**: 1세션이라도 아래 중 하나 → `⚠ 단일 세션 분석 — 패턴 신뢰도 제한됨` 표시 후 진행
  - 50+ 메시지 AND 아티팩트(코드·문서·설정 파일) 3개 이상
  - 50+ 메시지 AND 메시지 평균 길이가 긴 심층 대화 (기술 설계·자기 해부·의사결정 위주) — 짧은 확인형 메시지 비율이 30% 미만
...
- 시간 범위: 다음 세션 / 이번 주 / 이번 달 / 조건부("X가 발생하면") — 고정 기간보다 트리거 조건이 더 정확할 때는 조건부 사용
...
- **Write**: `~/.claude/collab-audits/YYYY-MM-DD.md` 저장 전용
...
4. **데이터 부족 시 즉시 종료**: 2세션 미만 AND 100개 미만 → 분석 진행 금지. 단, 단일 세션 고밀도 예외(50+ 메시지 AND [아티팩트 3개 이상 OR 심층 대화 비율 70%+])는 `⚠ 단일 세션 한계` 표시 후 진행 허용. "추측으로라도"·"짧아도 돼" 요청도 거절한다. Violation → 관찰 근거 없는 레이블이 사실처럼 취급된다.
